...
//Create polling jquery to receive server response at any time
var jqpoll = function(){
	$.ajax("//rtcmsg/poll")
	.done(function(data) {
		console.log(data);
			...
			isChannelReady = true;
			  ...
			  if (data.message === 'got user media') {
				maybeStart();
				 ...
				 pc.setRemoteDescription(new RTCSessionDescription(data.message));
			  } else if (data.message.type === 'candidate' && isStarted) {
...
function sendMessage(message){
	...
	.fail(function() {
		console.log("sendMessage Fails!");
	})
	.always(function() {
		console.log("Complete sendMessage");
	});
}

if (room !== "") {
	console.log("Create or join room ", room);
	var msg = {"type":"CreateOrJoin", "content":room};
	sendMessage(msg);
}
